...
def mapper():
    ...
    for line in reader:
		if line[0] == "id" or line[0] == "user_ptr_id": #skip headers
			continue
		elif len(line) == 5: #users file
			result = [line[0],'A'] + line[1:] #keep all fields
		elif len(line) == 19: #forum node file
			result = [line[3],'B'] + line[0:3] + line[5:10] #grab field 10 elements, except 4
		else: #something wrong, skip
			continue   
